...
export default function CommentsManager() {
  ...
  const act = async (id, action) => {
    try {
      ...
      const url = action === 'approve' ? `/api/comments/approve/${id}` : action === 'reject' ? `/api/comments/reject/${id}` : `/api/comments/${id}`
  ...
  return (
    <div className="space-y-6">
      <h1 className="text-2xl font-semibold">Comments Manager</h1>
      {error && <div className="text-sm text-red-600">{error}</div>}
      <div className="rounded-xl bg-white shadow-sm border border-gray-200 overflow-hidden">
        <div className="p-3 border-b text-sm text-gray-600">Moderate user comments</div>
            ...
            {loading ? (
              ...
              <tr key={t._id} className="hover:bg-gray-50">
                ...
                <td className="px-4 py-3 space-x-2">
                  ...
                  <button disabled={actingId===t._id} onClick={() => { if (confirm('Delete this comment?')) act(t._id, 'delete') }} className="px-3 py-1 rounded bg-red-50 text-red-700 hover:bg-red-100 disabled:opacity-60">{actingId===t._id && actingType==='delete' ? 'Deleting...' : 'Delete'}</button>
                </td>
              </tr>
            ))}
          </tbody>
